...
function Header({ errors, setErrors }) {
  const {
    headerTitle,
    setHeaderTitle,
    companyName,
  ...
  return (
    <>
      ...
      overflow-hidden outline-none border-black  ${
        errors.headerTitle ? "input-error" : ""
      }`}
                onChange={(e) => {
            ...
            {errors.headerTitle && (
              <p className="error">{errors.headerTitle}</p>
            )}

            <div className="w-1/3">
              <input
                type="text"
                className={`overflow-ellipsis overflow-hidden rounded px-8 outline-none p-4 mt-1 w-full border-bottom ${
      ...
      ${errors.email ? "input-error" : ""}`}
            ...
            <div className="mb-4 mt-4">
              <label htmlFor="monthYearInput" className="font-bold mr-5">
              ...
              <input
                ...
                value={
                  payMonth
                    ? new Date(payMonth).toLocaleDateString("en-CA", {
                        year: "numeric",
                        month: "2-digit",
                      })
                    : ""
                ...
                onChange={(e) =>
                  setPayMonth(e.target.value) && (errors.selectState = "")
                }
              />
            </div>
            ...
            {errors.selectState && (
              <p className="error-option">{errors.selectState}</p>
            )}
            <select
              name=""
            ...
            {errors.selectCity && (
              <p className="error-option">{errors.selectCity}</p>
            )}

            <div className="w-1/3">
              <div
                className={`mt-4 rounded outline-none ml-2 w-full  image_box`}
              >
                <label
                  htmlFor=""
                  className="mr-2 invisible sm:visible lg:visible"
                >
                  {" "}
                  Upload Logo:
                </label>
